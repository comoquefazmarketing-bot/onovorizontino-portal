...
// ── Tipos alinhados com o schema real (times_serie_b JOIN) ───
interface Time {
  id:          number;
  nome:        string;
  escudo_url:  string;
...
export default function TigreFCPage({ params }: { params: Promise<{ jogoId?: string }> }) {
  use(params); // resolve params (Next.js 15)
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        ...
        // 2. Próximo jogo e ranking em paralelo
        // cache: 'no-store' garante que nunca usa dados velhos
        ...
        if (resJogo?.jogos?.[0]) {
          setJogo(resJogo.jogos[0] as Jogo);
        }
        if (resRank.data) {
          setRanking(resRank.data as UsuarioRanking[]);
        }
      } catch (e) {
        console.error('[TigreFCPage] Erro ao carregar:', e);
      } finally {
        setIsLoading(false);
      }
    }
    ...
    // Realtime: atualiza automaticamente quando jogo muda
    const channel = sb
      .channel('tigre-fc-jogo-live')
      .on(
        'postgres_changes',
  ...
  // ── Timer countdown (trava 90min antes do jogo) ──────────
  useEffect(() => {
    ...
    const calcTime = () => {
      ...
      if (diff <= 0) {
        ...
        setMercadoFechado(true);
        return;
      }

      setMercadoFechado(false);
      setTimeLeft({
      ...
      <div className="max-w-md mx-auto px-4 -mt-20 relative z-20">
        ...
        {jogo ? (
          <section className="mb-20">
            <div className="bg-zinc-900/90 backdrop-blur-3xl rounded-[60px] border border-white/10 p-10 shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] relative overflow-hidden group">
              ...
              {/* Rodada */}
              <p className="text-center text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] mb-6">
              ...
              <div className="flex justify-between items-center mb-12 relative z-10">
                <div className="flex flex-col items-center flex-1">
                  ...
                  <p className="text-[10px] font-black uppercase text-zinc-400 text-center leading-tight">
                    {jogo.mandante.nome}
                  </p>
                </div>
                ...
                <div className="px-4 flex flex-col items-center">
                  ...
                  {jogo.data_hora && (
                    <p className="text-[9px] font-black text-yellow-500 text-center">
              ...
              {/* Local + Transmissão */}
              {jogo.local && (
                <p className="text-center text-[9px] text-zinc-600 font-bold mb-2">
                  📍 {jogo.local}
                </p>
              )}
              {jogo.transmissao && (
                <p className="text-center text-[9px] text-zinc-600 font-bold mb-8">
                  📺 {jogo.transmissao}
                </p>
              )}
              ...
              {/* CTA */}
              {mercadoFechado ? (
                <div className="relative flex items-center justify-center bg-zinc-800 text-zinc-500 py-7 rounded-[35px] font-[1000] uppercase italic text-sm">
                  🔒 MERCADO FECHADO
                </div>
              ) : (
                <Link
                  href={`/tigre-fc/escalar/${jogo.id}`}
                  className="relative flex items-center justify-center bg-[#F5C400] text-black py-7 rounded-[35px] font-[1000] uppercase italic text-sm shadow-[0_20px_40px_rgba(245,196,0,0.3)] hover:scale-[1.02] active:scale-95 transition-all overflow-hidden group"
                >
        ...
        <section className="mt-32">
          <div className="flex justify-between items-end mb-10 px-6">
            ...
            <Link href="/tigre-fc/ranking" className="text-[10px] font-black text-yellow-500 uppercase border-b border-yellow-500/30 pb-1 hover:text-white hover:border-white transition-all">
              Ver Todos
            </Link>
          </div>
