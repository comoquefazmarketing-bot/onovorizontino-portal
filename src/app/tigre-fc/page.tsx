...
// ─── Lookup: escudos dos 20 times da Série B 2026 ────────────────────────────
// Fonte: tabela `times` do Supabase (times_serie_b_rows.json)
...
/** Resolve o escudo pelo slug ou pelo nome do time */
function resolveEscudo(slugOrNome: string | undefined, fallbackUrl?: string): string {
  if (!slugOrNome) return fallbackUrl ?? PATA_LOGO;
  const slug = slugOrNome.toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  return ESCUDOS_SERIE_B[slug] ?? fallbackUrl ?? PATA_LOGO;
}
...
interface SofascoreInfo {
  eventId: number | null; homeTeam?: string; awayTeam?: string;
...
function TimerBlock({ value, label }: { value: string; label: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      <div style={{
        position: 'relative', width: 64, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
        background: 'linear-gradient(145deg, #0f0f0f, #1a1a00)', border: '1px solid rgba(245,196,0,0.2)', borderRadius: 14,
        boxShadow: '0 0 16px rgba(245,196,0,0.06), inset 0 1px 0 rgba(255,255,255,0.03)',
      }}>
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {Array.from({ length: 14 }).map((_, i) => (
        <motion.div key={i} style={{
          ...
          background: '#F5C400', borderRadius: '50%', left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, opacity: 0,
        }}
...
function SectionLabel({ sub, title }: { sub: string; title: string }) {
  return (
    <div style={{ marginBottom: 28, textAlign: 'center' }}>
      ...
      <h2 style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: 36, fontWeight: 900, color: '#fff', textTransform: 'uppercase', letterSpacing: -1, lineHeight: 1, margin: 0 }}>
        {title}
      </h2>
    </div>
  );
...
export default function TigreFCPage({ params }: { params: Promise<{ jogoId?: string }> }) {
  ...
  const [sofascore,      setSofascore]      = useState<SofascoreInfo | null>(null);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (resJogo?.jogos?.length > 0) {
        ...
        // Garante logo correta pelo slug
        if (j.visitante && !j.visitante.escudo_url?.startsWith('http')) {
          j.visitante.escudo_url = resolveEscudo(j.visitante.slug ?? j.visitante.nome);
        }
        if (j.mandante && !j.mandante.escudo_url?.startsWith('http')) {
          j.mandante.escudo_url = resolveEscudo(j.mandante.slug ?? j.mandante.nome);
        }
        setJogo(j);
      } else {
        setJogo({
      ...
      // Sofascore último jogo
      try {
        const sfData = await fetch('/api/tigre-fc/sofascore-ultimo-jogo').then(r => r.json());
        setSofascore(sfData);
      } catch { /* sem sofascore */ }
    }
    init();
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#050505', color: '#fff', overflowX: 'hidden', fontFamily: "'Barlow Condensed', system-ui, sans-serif" }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '80px 24px 56px', textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 } as TargetAndTransition} animate={{ opacity: 1, y: 0 } as TargetAndTransition} transition={{ delay: 0.2, duration: 0.5, ease: EASE }}>
            ...
            <h1 style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: 84, fontWeight: 900, letterSpacing: -4, lineHeight: 0.85, textTransform: 'uppercase', margin: '0 0 14px' }}>
              <span className="text-shimmer">TIGRE</span><br />
          ...
          <motion.div initial={{ opacity: 0 } as TargetAndTransition} animate={{ opacity: 1 } as TargetAndTransition} transition={{ delay: 0.5 }}
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 999, background: mercadoAberto ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${mercadoAberto ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
              <span style={{ fontSize: 7, fontWeight: 900, color: mercadoAberto ? '#22C55E' : '#EF4444', letterSpacing: 3, textTransform: 'uppercase' }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px 20px' }}>
                  ...
                  <div style={{ padding: '24px 22px 28px', position: 'relative', zIndex: 2 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, gap: 8 }}>
                        <div style={{ width: 72, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle, rgba(245,196,0,0.12), transparent 70%)', borderRadius: '50%' }}>
                          <img src={resolveEscudo(jogo.mandante?.slug ?? jogo.mandante?.nome, jogo.mandante?.escudo_url)}
                            onError={e => { (e.target as HTMLImageElement).src = PATA_LOGO; }}
                            style={{ width: 58, height: 58, objectFit: 'contain', filter: 'drop-shadow(0 0 12px rgba(245,196,0,0.35))' }} alt={jogo.mandante?.nome} />
                        </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '0 6px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 18, fontWeight: 900, color: '#F5C400', marginBottom: 14, textShadow: '0 0 8px rgba(245,196,0,0.4)' }}>:</span>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, gap: 8 }}>
                        <div style={{ width: 72, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle, rgba(255,255,255,0.04), transparent 70%)', borderRadius: '50%' }}>
                          <img src={resolveEscudo(jogo.visitante?.slug ?? jogo.visitante?.nome, jogo.visitante?.escudo_url)}
                            onError={e => { (e.target as HTMLImageElement).src = PATA_LOGO; }}
                            style={{ width: 58, height: 58, objectFit: 'contain', filter: 'drop-shadow(0 4px 10px rgba(0,0,0,0.5))' }} alt={jogo.visitante?.nome} />
                        </div>
                    ...
                    <div className="hud-pulse" style={{ borderRadius: 18 }}>
                      <Link href={`/tigre-fc/escalar/${jogo.id}`} style={{
                        ...
                        background: mercadoAberto ? 'linear-gradient(135deg, #F5C400 0%, #D4A200 100%)' : '#1a1a1a',
                        color: mercadoAberto ? '#000' : '#333',
                        fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: 13, fontWeight: 900, letterSpacing: 3, textTransform: 'uppercase', textDecoration: 'none',
                        boxShadow: mercadoAberto ? '0 8px 28px rgba(245,196,0,0.25)' : 'none',
        ...
        {/* ── SOFASCORE — ANÁLISE DA ÚLTIMA RODADA ── */}
        {sofascore?.eventId && (
          <motion.section {...stagger(2)} style={{ marginTop: 52 }}>
            <SectionLabel sub="ANÁLISE TÁTICA" title="ÚLTIMA RODADA" />

            <div style={{ position: 'relative', borderRadius: 24, overflow: 'hidden', border: '1px solid rgba(245,196,0,0.12)', background: '#080808' }}>
              <div style={{ height: 2, background: 'linear-gradient(90deg, transparent, #F5C400, transparent)' }} />

              {/* Header */}
              <div style={{ padding: '14px 18px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 900, color: '#fff', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    {sofascore.homeTeam} <span style={{ color: '#F5C400' }}>{sofascore.homeScore} × {sofascore.awayScore}</span> {sofascore.awayTeam}
                  </div>
                  <div style={{ fontSize: 8, color: '#333', marginTop: 3, fontWeight: 700 }}>
                    Use os dados para montar sua escalação ↓
                  </div>
                </div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 8px', borderRadius: 999, background: 'rgba(245,196,0,0.08)', border: '1px solid rgba(245,196,0,0.2)' }}>
                  <span style={{ fontSize: 7, fontWeight: 900, color: '#F5C400', letterSpacing: 2, textTransform: 'uppercase' }}>Sofascore</span>
                </div>
              </div>

              {/* Widget tabs */}
              <SofascoreWidget eventId={sofascore.eventId} />
            </div>
          </motion.section>
        )}
        ...
        <motion.section {...stagger(3)} style={{ marginTop: 56 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {ranking.length === 0 ? (
              ...
              return (
                <motion.div key={u.id}
                  initial={{ opacity: 0, x: i % 2 === 0 ? -16 : 16 } as TargetAndTransition}
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', gap: 12, padding: isFirst ? '18px' : '13px 16px',
                    borderRadius: isFirst ? 22 : 16, cursor: 'pointer', position: 'relative', overflow: 'hidden',
                    background: isFirst ? 'linear-gradient(135deg, #1a1400, #2a1f00, #1a1400)' : 'rgba(255,255,255,0.02)',
                  ...
                  <div style={{ width: 32, textAlign: 'center', flexShrink: 0 }}>
                    {isTop3 ? <span style={{ fontSize: 18 }}>{MEDALS[i]}</span>
                      : <span style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: 20, fontWeight: 900, fontStyle: 'italic', color: '#222' }}>{String(i + 1).padStart(2, '0')}</span>}
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: isFirst ? 20 : 16, fontWeight: 900, fontStyle: 'italic', color: isFirst ? '#F5C400' : '#fff', textTransform: 'uppercase', letterSpacing: -0.5, lineHeight: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {u.apelido || u.nome}
                    </div>
                    <div style={{ fontSize: 7, fontWeight: 900, color: isFirst ? 'rgba(245,196,0,0.4)' : '#222', letterSpacing: 2, textTransform: 'uppercase', marginTop: 3 }}>
                      {RANK_LABELS[i]}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: "'Barlow Condensed', Impact, sans-serif", fontSize: isFirst ? 34 : 26, fontWeight: 900, fontStyle: 'italic', color: isFirst ? '#F5C400' : isTop3 ? '#fff' : '#333', lineHeight: 1, textShadow: isFirst ? '0 0 16px rgba(245,196,0,0.35)' : 'none' }}>
                      {u.pontos_total || 0}
                    </div>
          ...
          <Link href="/tigre-fc/ranking" style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            padding: '13px', borderRadius: 14, marginTop: 12,
            background: 'transparent', border: '1px solid rgba(245,196,0,0.12)',
            color: '#F5C400', fontSize: 10, fontWeight: 900, letterSpacing: 3, textTransform: 'uppercase', textDecoration: 'none',
          }}>
...
// ─── Sofascore Widget com tabs ────────────────────────────────────────────────
function SofascoreWidget({ eventId }: { eventId: number }) {
  const [activeTab, setActiveTab] = useState<'lineups' | 'ratings' | 'stats'>('lineups');
  const TABS: { id: 'lineups' | 'ratings' | 'stats'; label: string; icon: string }[] = [
    { id: 'lineups', label: 'Escalação', icon: '⚽' },
    { id: 'ratings', label: 'Notas', icon: '⭐' },
    { id: 'stats',   label: 'Stats', icon: '📊' },
  ];
  const SRC: Record<string, string> = {
    lineups: `https://widgets.sofascore.com/pt-BR/embed/lineups?id=${eventId}&widgetTheme=dark`,
    ratings: `https://widgets.sofascore.com/pt-BR/embed/ratings?id=${eventId}&widgetTheme=dark`,
    stats:   `https://widgets.sofascore.com/pt-BR/embed/statistics?id=${eventId}&widgetTheme=dark`,
  };
  ...
  return (
    <>
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            style={{
              flex: 1, padding: '10px 4px', background: 'none', cursor: 'pointer',
              borderBottom: activeTab === t.id ? '2px solid #F5C400' : '2px solid transparent',
              color: activeTab === t.id ? '#F5C400' : '#333',
      ...
      <div style={{ position: 'relative', height: 520, background: '#080808' }}>
        <div style={{ position: 'absolute', top: 8, left: 8, fontSize: 7, fontWeight: 900, color: '#1a1a1a', letterSpacing: 2, textTransform: 'uppercase' }}>
          LIVE RADAR SYSTEM
        </div>
        <iframe
          key={activeTab}
