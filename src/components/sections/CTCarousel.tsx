...
export default function CTCarousel() {
  // Caminho exato baseado na sua pasta public/assets/images
        ...
        <img 
          ...
          onError={(e) => {
            ...
            if (target.src.includes(".jpg")) {
              target.src = target.src.replace(".jpg", "");
            }
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent flex flex-col justify-end p-6">
          <h2 className="text-white text-xl md:text-3xl font-black uppercase italic leading-none drop-shadow-xl">
            {photos[active].title}
          </h2>
        ...
        {photos.map((img, i) => (
          <button
            ...
            className={`relative min-w-[90px] h-[60px] rounded-xl overflow-hidden border-2 transition-all ${
              active === i ? 'border-yellow-500 scale-105 z-10' : 'border-transparent opacity-30 hover:opacity-100'
            }`}
          >
            <img 
              src={`${localPath}${img.name}`} 
              className="w-full h-full object-cover" 
              alt="Mini"
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                if (target.src.includes(".jpg")) target.src = target.src.replace(".jpg", "");
              }}
            />
          </button>
        ))}
