...
// ✅ SERVER COMPONENT — sem 'use client', sem useEffect, sem fetch de API
// Busca direto no Supabase no servidor → página chega ao browser já renderizada

import Link from 'next/link';
import Image from 'next/image';
import { createClient } from '@/utils/supabase/server';

export const revalidate = 300; // revalida cache a cada 5 min no servidor
...
function CardDestaque({ post }: { post: Postagem }) {
  return (
    <Link
      ...
      className="group relative block rounded-2xl overflow-hidden aspect-[16/9] bg-zinc-900 border border-white/5 hover:border-yellow-500/40 transition-all duration-300"
    >
      {post.imagem_capa && (
        <Image
...
export default async function PostagensGrid() {
  ...
  if (error) console.error('[PostagensGrid]', error.message);
  if (!postagens || postagens.length === 0) return null;

  const [destaque, ...resto] = postagens as Postagem[];
    ...
    <section className="max-w-7xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-white text-xl font-black italic uppercase tracking-tighter">Últimas Notícias</h2>
        </div>
