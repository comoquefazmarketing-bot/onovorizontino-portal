...
const CORNETAS = [
  ...
  '😂 Tá escalando com os olhos fechados?',
  '🎯 Chuta que é macumba essa escalação!',
];
...
function MiniPlayerCard({ player, isCaptain, isHero }: { player: Player; isCaptain: boolean; isHero: boolean }) {
  return (
    <motion.div whileHover={{ scale: 1.15 }} className="flex flex-col items-center relative z-10">
      <div className={`relative w-10 h-14 rounded-md overflow-hidden border-2 shadow-lg ${
        ...
        {isCaptain && (
          <div className="absolute top-0 right-0 bg-yellow-500 text-black text-[6px] font-black px-1 rounded-bl leading-tight">C</div>
        )}
        {isHero && (
          <div className="absolute top-0 right-0 bg-cyan-400 text-black text-[6px] font-black px-1 rounded-bl leading-tight">H</div>
        )}
...
export default function TigreFCPerfilPublico({
  ...
  if (!dados) return (
    <div className="flex flex-col items-center justify-center h-64 bg-zinc-950 rounded-[2rem] border border-zinc-900 p-8 text-center gap-3">
      ...
      <p className="text-zinc-500 font-black uppercase tracking-tighter italic text-sm">
        Este torcedor ainda não escalou o Tigre!
      </p>
      {onClose && (
  ...
  const playersEscalados = slots.filter(s => lineup[s.id]).length;
  ...
  return (
    <div className="relative bg-black rounded-[2rem] border border-zinc-800 overflow-hidden shadow-2xl">
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 w-8 h-8 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-500 transition text-lg font-light"
        >
          ✕
        </button>
      )}
      ...
      <div className="p-6 border-b border-zinc-800 bg-gradient-to-b from-zinc-900 to-black flex items-center gap-4">
        <div className="relative shrink-0">
          <img
            src={perfil.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(perfil.display_name ?? 'torcedor')}`}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-xl font-[1000] italic uppercase tracking-tighter text-white truncate">
              {perfil.display_name ?? 'Torcedor'}
            </h2>
            {isSelfView && (
              <span className="text-[8px] bg-yellow-500 text-black px-2 py-0.5 rounded-full font-black uppercase shrink-0">
                VOCÊ
              </span>
            )}
          </div>
          ...
          <p className="text-zinc-700 text-[9px] mt-0.5 font-medium">
            {playersEscalados}/11 escalados • {dados.formacao ?? '4-2-3-1'}
          </p>
        </div>

        {dados.score_locked && (
          <div className="text-right shrink-0">
            <p className="text-[8px] text-zinc-500 font-black uppercase mb-1">Palpite</p>
            <div className="flex items-center gap-1.5">
              <span className="text-2xl font-black italic text-yellow-500">{dados.score_tigre}</span>
              <span className="text-xs text-zinc-700 font-black">X</span>
        ...
        <div className="relative w-full aspect-[1/1.2] rounded-3xl overflow-hidden bg-[#1e5c1e] border-2 border-white/5 shadow-inner">
          ...
          <div className="absolute inset-0 pointer-events-none" style={{
            background: `
              ...
              radial-gradient(circle at 85% 10%, rgba(200,240,255,0.1) 0%, transparent 50%)
            `,
          }} />
          ...
          <AnimatePresence>
            {corneta && (
              <motion.div
                ...
                className="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-center"
              >
