...
const PLAYERS: Record<number, { id:number; name:string; short:string; num:number; pos:string; foto:string }> = {
...
const FORMATIONS_SLOTS: Record<string, typeof SLOTS_433> = {
  '4-3-3': SLOTS_433,
  '4-4-2': [
    { id:'gk',x:50,y:88},{id:'rb',x:82,y:70},{id:'cb1',x:62,y:70},{id:'cb2',x:38,y:70},
    {id:'lb',x:18,y:70},{id:'rm',x:80,y:50},{id:'cm1',x:60,y:50},{id:'cm2',x:40,y:50},
    {id:'lm',x:20,y:50},{id:'st1',x:64,y:22},{id:'st2',x:36,y:22},
  ],
  '3-5-2': [
    {id:'gk',x:50,y:88},{id:'cb1',x:70,y:72},{id:'cb2',x:50,y:75},{id:'cb3',x:30,y:72},
    {id:'rb',x:86,y:52},{id:'cm1',x:68,y:50},{id:'cm2',x:50,y:46},{id:'cm3',x:32,y:50},
    {id:'lb',x:14,y:52},{id:'st1',x:64,y:22},{id:'st2',x:36,y:22},
  ],
  '4-2-3-1': [
    {id:'gk',x:50,y:88},{id:'rb',x:82,y:70},{id:'cb1',x:62,y:70},{id:'cb2',x:38,y:70},
    {id:'lb',x:18,y:70},{id:'dm1',x:64,y:57},{id:'dm2',x:36,y:57},{id:'rm',x:76,y:38},
    {id:'am',x:50,y:36},{id:'lm',x:24,y:38},{id:'st',x:50,y:18},
  ],
};
...
export default function TigreFCPerfilPublico({ targetUserId, jogoId, meuId, onClose }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: u }, { data: esc }, { data: pal }, { data: coments }] = await Promise.all([
      ...
      setPerfil(u); setEscalacao(esc); setPalpite(pal);
  ...
  const enviarComentario = async () => {
    if (!novoComent.trim() || !meuId || enviando) return;
    setEnviando(true);
  ...
  const slots = FORMATIONS_SLOTS[escalacao?.formacao || '4-3-3'] || SLOTS_433;
    ...
    <div style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(0,0,0,0.85)', display:'flex', alignItems:'flex-end', justifyContent:'center' }}
      ...
      <div style={{ width:'100%', maxWidth:480, background:'#0a0a0a', borderRadius:'20px 20px 0 0', border:'1px solid #1a1a1a', maxHeight:'92vh', overflow:'hidden', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ background:'#F5C400', padding:'14px 16px', display:'flex', alignItems:'center', gap:12, flexShrink:0 }}>
          ...
          <div style={{ fontWeight:900, fontSize:15, color:'#1a1a1a' }}>Perfil do Torcedor</div>
        </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ padding:'20px 16px', display:'flex', alignItems:'center', gap:14, borderBottom:'1px solid #111' }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:18, fontWeight:900, color:'#fff' }}>{perfil.apelido || perfil.nome}</div>
              ...
              {escalacao ? (
                ...
                <div style={{ padding:24, textAlign:'center', color:'#555', fontSize:13 }}>
                  {targetUserId === meuId ? 'Você ainda não escalou nessa rodada.' : 'Escalação não divulgada ainda.'}
                </div>
              )}
              ...
              <div style={{ padding:'0 16px 16px', borderTop:'1px solid #111', marginTop:8 }}>
                    ...
                    {comentarios.map((c: any) => {
                      ...
                      return (
                        <div key={c.id} style={{ display:'flex', gap:10, alignItems:'flex-start' }}>
                          {autor?.avatar_url ? (
                            <img src={autor.avatar_url} style={{ width:32, height:32, borderRadius:'50%', border:`1.5px solid ${cor}`, objectFit:'cover', flexShrink:0 }} />
                          ) : (
                ...
                {meuId ? (
                  <div style={{ display:'flex', gap:8 }}>
                    <input
                      value={novoComent}
                      onChange={e => setNovoComent(e.target.value.slice(0, 280))}
