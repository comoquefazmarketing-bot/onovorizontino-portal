...
const COPYS = [
  { titulo: 'Você já discutiu a escalação no grupo.', texto: 'Agora tem ranking pra provar quem manja mais. É grátis. É do Tigre.', cta: 'Entrar no jogo →' },
  { titulo: 'Todo mundo fala que escalaria diferente.', texto: 'Agora a gente vai ver. Monta o time, crava o placar, sobe no ranking.', cta: 'Escala aí →' },
  { titulo: 'Chama a galera do grupo.', texto: 'Quem conhece mais o Tigre vai aparecer no ranking. Vai encarar?', cta: 'Bora jogar →' },
];
...
export default function TigreFCButton() {
  ...
  return (
    <>
      <style>{`
        @keyframes tigre-pulse { 0%,100%{box-shadow:0 0 0 0 rgba(245,196,0,0.4)} 50%{box-shadow:0 0 0 12px rgba(245,196,0,0)} }
        @keyframes tigre-in { from{opacity:0;transform:scale(0.8) translateY(10px)} to{opacity:1;transform:scale(1) translateY(0)} }
      ...
      {expanded && (
        <div className="tigre-popup" style={{ position:'fixed', bottom:90, right:16, zIndex:9998, background:'linear-gradient(135deg,#111,#1a1200)', border:'1px solid #F5C400', borderRadius:16, padding:'16px', width:240, boxShadow:'0 8px 32px rgba(0,0,0,0.6)' }}>
          ...
          <p style={{ fontSize:13, color:'#aaa', lineHeight:1.5, marginBottom:6, fontWeight:700 }}>{copy.titulo}</p>
          <p style={{ fontSize:12, color:'#666', lineHeight:1.5, marginBottom:14 }}>{copy.texto}</p>
          <Link href="/tigre-fc/sobre" onClick={() => setExpanded(false)}
