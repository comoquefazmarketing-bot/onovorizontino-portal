...
function Countdown({ dataHora }: { dataHora: string }) {
  ...
  const block = (val: string, lbl: string, red = false) => (
    <div style={{ background:'#080808', border:'1px solid rgba(245,196,0,0.18)', borderRadius:10, padding:'8px 10px', textAlign:'center', minWidth:58, boxShadow:'inset 0 1px 0 rgba(245,196,0,0.06)' }}>
      <span style={{ fontFamily:"'Barlow Condensed',monospace", fontSize:46, fontWeight:900, lineHeight:1, display:'block', fontVariantNumeric:'tabular-nums',
        color: red ? C.red : '#fff',
...
function Escudo({ src, alt, novo }: { src: string | null; alt: string; novo?: boolean }) {
  ...
  return (
    <div style={{
      width:76, height:76, background:'#0d0d0d', borderRadius:18,
...
function StatCard({ lbl, val, sub, color, border }: {
  lbl: string; val: string; sub?: string;
...
export default function JumbotronJogo({ jogo, stats = {}, mercadoFechado = false }: Props) {
  ...
  return (
    <div style={{
      ...
      position:'relative', border:'1px solid rgba(245,196,0,0.22)',
      ...
      <style>{`
        ...
        @keyframes red-lbl{0%,100%{opacity:1}50%{opacity:0.6}}
      ...
      {/* LED textures */}
      <div style={{ position:'absolute', inset:0, pointerEvents:'none', backgroundImage:'radial-gradient(circle,rgba(245,196,0,0.04) 1px,transparent 1px)', backgroundSize:'4px 4px' }} />
      <div style={{ position:'absolute', inset:0, pointerEvents:'none', backgroundImage:`repeating-linear-gradient(45deg,transparent,transparent 12px,rgba(245,196,0,0.016) 12px,rgba(245,196,0,0.016) 14px),repeating-linear-gradient(-45deg,transparent,transparent 18px,rgba(245,196,0,0.012) 18px,rgba(245,196,0,0.012) 20px)` }} />

      {/* Scan bar */}
      <div style={{ height:2, background:`linear-gradient(90deg,transparent,${C.gold},#fff,${C.cyan},transparent)`, backgroundSize:'200%', animation:'scan 2.5s linear infinite' }} />
      ...
      <div style={{ position:'relative', zIndex:1, padding:'16px 14px 18px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, paddingBottom:10, borderBottom:'1px solid rgba(245,196,0,0.1)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:5 }}>
            <span style={{ width:6, height:6, borderRadius:'50%', background:C.cyan, boxShadow:`0 0 6px ${C.cyan},0 0 14px ${C.cyan}`, display:'inline-block', animation:'blink 1s ease-in-out infinite' }} />
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, gap:6 }}>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:7, flex:1 }}>
            ...
            <span style={{ fontSize:10, fontWeight:900, letterSpacing:'0.1em', color:'rgba(255,255,255,0.4)', textAlign:'center', lineHeight:1.2 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:7, flex:1 }}>
            ...
            <span style={{ fontSize:10, fontWeight:900, letterSpacing:'0.1em', color:C.gold, textShadow:`0 0 8px rgba(245,196,0,0.4)`, textAlign:'center', lineHeight:1.2 }}>
        ...
        {/* ESTÁDIO + TV */}
        <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:10, padding:'9px 12px', marginBottom:10 }}>
          {jogo.local && (
        ...
        {/* XP BADGE */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, background:`rgba(191,95,255,0.1)`, border:`1px solid rgba(191,95,255,0.2)`, borderRadius:8, padding:'7px 12px', marginBottom:12 }}>
        ...
        {/* CTA */}
        {mercadoFechado ? (
          <div style={{ width:'100%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, color:'rgba(255,255,255,0.25)', fontFamily:"'Barlow Condensed',sans-serif", fontSize:11, fontWeight:900, letterSpacing:'0.22em', padding:16, textAlign:'center' }}>
            🔒 MERCADO FECHADO
          </div>
        ) : (
          <Link href={`/tigre-fc/escalar/${jogo.id}`} style={{
            ...
            borderRadius:12, color:'#000', fontFamily:"'Barlow Condensed',sans-serif",
        ...
        <div style={{ fontSize:8, fontWeight:700, letterSpacing:'0.15em', color:'rgba(255,255,255,0.18)', textAlign:'center', marginTop:8 }}>
          Mercado fecha 90min antes do jogo
        </div>

        {/* STATS 2×2 */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:14, paddingTop:12, borderTop:'1px solid rgba(255,255,255,0.05)' }}>
          <StatCard
            lbl="CAPITÃO · ÚLT. RD"
        ...
        {/* RANKING */}
        {ranking.length > 0 && (
          <div style={{ marginTop:10, background:`linear-gradient(160deg,rgba(191,95,255,0.06),transparent 70%)`, border:`1px solid rgba(191,95,255,0.15)`, borderRadius:10, padding:'10px 12px' }}>
            <div style={{ fontSize:8, fontWeight:900, letterSpacing:'0.35em', color:'rgba(191,95,255,0.5)', marginBottom:8, paddingBottom:5, borderBottom:'1px solid rgba(191,95,255,0.1)' }}>
              RANKING · TOP 5
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              {ranking.slice(0,5).map((r, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  <span style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.65)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            ...
            {participantes > 0 && (
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:7, paddingTop:6, borderTop:'1px solid rgba(191,95,255,0.08)' }}>
                <span style={{ fontSize:8, color:C.cyan, letterSpacing:'0.12em', fontWeight:700 }}>TORCEDORES ESCALANDO</span>
                <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:20, fontWeight:900, color:C.cyan, textShadow:C.gCyan }}>{participantes}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:5, marginTop:10 }}>
          {[
            ...
            { val: mediaSofaTime != null ? mediaSofaTime.toFixed(2) : '—', lbl:'MÉD SOFA', color:C.gold,   bg:'rgba(245,196,0,0.03)',   border:'rgba(245,196,0,0.08)' },
            ...
            <div key={i} style={{ textAlign:'center', background:s.bg, border:`1px solid ${s.border}`, borderRadius:6, padding:'7px 3px' }}>
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:16, fontWeight:900, fontStyle:'italic', lineHeight:1, color:s.color, textShadow:`0 0 8px ${s.color}88` }}>{s.val}</div>
              <div style={{ fontSize:6, fontWeight:700, letterSpacing:'0.12em', color:'rgba(255,255,255,0.2)', marginTop:3 }}>{s.lbl}</div>
            </div>
          ))}
        </div>
