...
async function fetchUltimaRodada(): Promise<UltimaRodada> {
  ...
  const { count } = await supabase.from('tigre_fc_escalacoes').select('*', { count: 'exact', head: true });

  return { capitao, heroi, maisEscalado: null, ranking, participantes: count ?? 0 };
}
...
function Escudo({ src, alt, novo }: { src: string | null; alt: string; novo?: boolean }) {
  ...
  return (
    <div style={{ width:90, height:90, background:'#0d0d0d', borderRadius:22, flexShrink:0,
...
function StatCard({ lbl, val, sub, color, border }: {
  ...
  return (
    <div style={{ background:'rgba(255,255,255,0.06)', border:`1px solid ${border}`, borderRadius:12, padding:'12px' }}>
      <div style={{ fontSize:10, fontWeight:900, letterSpacing:'0.1em', color, marginBottom:6, opacity: 0.9 }}>{lbl}</div>
      <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:28, fontWeight:900, fontStyle:'italic', lineHeight:1, color: '#fff' }}>{val}</div>
...
export default function JumbotronJogo({ jogo, stats = {}, mercadoFechado = false }: Props) {
  ...
  return (
    <div style={{ fontFamily:"'Barlow Condensed', sans-serif", background:'#000',
      borderRadius:24, overflow:'hidden', position:'relative',
      border:'1px solid #333', maxWidth:460, width:'95%', margin:'0 auto',
      ...
      <div style={{ position:'relative', zIndex:1, padding:'24px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:20 }}>
          <StatCard
            ...
            color={C.gold} border="rgba(245,196,0,0.3)"
          />
          <StatCard
            lbl="HERÓI"
            val={loadingUltima ? '...' : heroi ? heroi.pts.toFixed(1) : '—'}
            sub={heroi ? heroi.nome : '—'}
            color={C.red} border="rgba(255,45,85,0.3)"
          />
        </div>
        ...
        <div style={{ marginTop:16, background:'#0a0a0a', border:`2px solid ${C.purple}`, borderRadius:16, padding:'16px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {ranking.slice(0,5).map((r, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px', borderRadius:10, background: i === 0 ? 'rgba(191,95,255,0.2)' : '#111' }}>
