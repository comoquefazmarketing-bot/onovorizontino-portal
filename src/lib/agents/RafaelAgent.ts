...
export interface MetricaBloco {
  label:    string;
  valor:    number;
  variacao?: number;    // diferença em relação ao período anterior (absoluta)
  variacao_pct?: number; // % de variação
  detalhe?: string;
}
...
// Estas funções são chamadas pela rota de API, não pelo cliente.

export interface ColetaParams {
  supabaseUrl:  string;
  anonKey:      string;
  inicioPeriodo: string;   // ISO — início da semana atual
  fimPeriodo:    string;   // ISO — fim da semana atual
  inicioAnterior: string;  // ISO — início da semana anterior
  fimAnterior:    string;  // ISO — fim da semana anterior
}
...
export const RafaelAgent = {
  ...
  /** Coleta métricas e monta o relatório semanal. */
  coletarMetricas,
